...
export default function PartyAProfileView() {
  ...
  const { user } = useAuth()
  const paths = getPartyAProfilePaths(user?.role)
    ...
    <div className="mx-auto max-w-4xl space-y-6">
      ...
      <section className="rounded-xl border border-slate-200 bg-white p-6 shadow-sm">
        ...
        <dl className="grid gap-4 sm:grid-cols-2">
          ...
          <InfoRow label="Tax ID (NTN)" value={profile.tax_id} />
          <InfoRow label="SECP Number" value={profile.secp_number} />
          <InfoRow label="PSW ID" value={profile.psw_id} />
          <InfoRow label="HS CODE(s)" value={profile.hs_codes} />
          <InfoRow
            label="FBR Certificate Issue Date"
            value={formatDate(profile.fbr_certificate_issue_date)}
          />
          <InfoRow label="FBR Tax Office" value={profile.fbr_tax_office} />
          <InfoRow
            label="SECP Incorporation Date"
            value={formatDate(profile.secp_incorporation_date)}
          />
      ...
      <section className="space-y-4">
        ...
        <div className="rounded-xl border border-slate-200 bg-white p-6 shadow-sm">
          <h5 className="font-semibold text-slate-800">FBR Taxpayer Registration Certificate</h5>
          {fbrDoc?.original_filename && (
        ...
        <div className="rounded-xl border border-slate-200 bg-white p-6 shadow-sm">
          <h5 className="font-semibold text-slate-800">SECP Certificate of Incorporation</h5>
          {secpDoc?.original_filename && (
