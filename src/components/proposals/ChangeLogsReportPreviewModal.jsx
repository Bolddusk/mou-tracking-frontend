...
import { parseChangeLogsCsv } from '../../utils/changeLogReport'

export default function ChangeLogsReportPreviewModal({
  open,
  csvText,
  onClose,
  onDownload,
  downloadLoading = false,
  title = 'Change Logs Report Preview',
  subtitle = 'Same data as export — preview is CSV; download may be Excel when filters are applied.',
}) {
  const parsed = useMemo(() => parseChangeLogsCsv(csvText || ''), [csvText])
          ...
          {parsed.summaryLines.length > 0 && (
            <div className="mb-4 rounded-lg border border-slate-200 bg-slate-50 px-4 py-3 text-sm text-slate-700">
              {parsed.summaryLines.map((line) => (
                <p key={line} className="font-mono text-xs leading-relaxed text-slate-600">
                  {line}
                </p>
              ))}
            </div>
          ...
          {parsed.headers.length > 0 ? (
            <div className="overflow-x-auto rounded-xl border border-slate-200">
                  ...
                  {parsed.rows.map((row, rowIndex) => (
                    <tr key={rowIndex} className="hover:bg-slate-50/80">
                      {parsed.headers.map((header, cellIndex) => (
                        <td
                          key={`${rowIndex}-${header}`}
                          className="max-w-[240px] px-3 py-2 align-top text-slate-700"
                          title={row[cellIndex]}
                        >
              ...
              {parsed.rows.length === 0 && (
                <p className="px-4 py-8 text-center text-sm text-slate-500">No log rows in this report.</p>
              )}
            </div>
          ) : (
