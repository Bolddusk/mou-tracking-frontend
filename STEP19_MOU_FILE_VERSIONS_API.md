...
- Show **current** file from `GET .../mou/status` (`mou_file_url`)
- Show **version badge**: `v{version}` from latest upload response, or derive from `versions[0].version_number`
- Link **“View history”** → `GET .../mou/versions`
...
2. If ack_reset === true: show warning toast with message, refetch GET .../mou/status, update UI to show both parties un-acknowledged (not signed)
3. Add "Version history" section/modal on MOU panel:
   - Fetch GET .../mou/versions on open
   - Table: version #, file download link, uploaded_by_name, uploaded_at
   - Mark latest version as "Current"
4. Show current version badge (e.g. "v2") next to MOU file link after upload
5. Reuse one shared MouVersionHistory component for proposal and match contexts (prop: entityType, entityId)
6. Do not allow UI to show "Signed" after ack_reset without refetching status
